...
Note that each of the `validate_*()` functions contain standard objects in their call environment which are passed to any custom check function and assigned automatically to any argument with the same name. They therefore do not need explicit inclusion in the `args` configuration. Alternatively, they are also available to be passed to any argument explicitly in the validation config file if required.
...
- **`validate_model_data`:** 
  ...
  - `tbl_chr` an all character version of `tbl`.
  - `file_path`: character string of path to file being validated relative to the `model-output` directory.
  - `hub_path`: character string of path to hub.
  - `round_id`: character string of `round_id`
  - `file_meta`: named list containing `round_id`, `team_abbr`, `model_abbr` and `model_id` details.
  - `round_id_col`: character string of name of `tbl` column containing `round_id` information.
  - `output_type_id_datatype`: character string of the data type of the `output_type_id` column in the `tbl`.
  - `derived_task_ids`: `NULL` or character vector of derived task IDs names.
  - `compound_taskid_set`: named list of `compound_taskid_set`, one element for each model task, as detected by `get_tbl_compound_taskid_set()` from `tbl`. 
