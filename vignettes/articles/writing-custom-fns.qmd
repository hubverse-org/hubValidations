...
The easiest way to make the hub config available within your function is to pass in the `hub_path` caller environment object by specifying it as a function argument and then use `hubUtils::read_config(hub_path)` to read in the `tasks.json` configuration file.

You can add a `config = TRUE` argument when creating the custom check function with `create_custom_check()` to include the `hub_path` argument and read in the hub configuration file in you custom check function skeleton.
