...
The function requires a name for the new custom check function, creates a `.R` script file in `src/validations/R` directory in the root of the hub, the recommended location. The script contains basic skeleton code to create a named custom check function. 

The output of `create_custom_check()` can also be parametarised through a number of arguments to include additional template code snippets.
...
When writing your functions you might want to use functions from other packages. All `hubValidations` exported functions are available for use in your custom check functions. 

## Available dependencies

In addition, functions from the following `hubValidation` dependencies are also generally available, both locally and in the hubverse `validate-submission` GitHub Action (once `hubValidations` is installed).

```{r, echo=FALSE}
pkgs <- suppressMessages(pak::pkg_deps(".")[,c("package", "version")])
```
...
If your custom check function requires additional packages, the easiest way to ensure they are available is to make sure they are installed in the `hubverse/validate-submission` GitHub Action workflow is to add them to the packages list in the `setup-r-dependencies` step.

In the following pseudo exam
