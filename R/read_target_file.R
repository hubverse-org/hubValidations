...
read_target_file <- function(target_file_path, hub_path,
  ...
  if (!file_ext %in% valid_ext) {
    cli::cli_abort("File cannot be read. File extension must be one of {.val {valid_ext}}, not {.val {file_ext}}")
  }
  ...
  if (coerce_types == "hub") {
    schema <- switch(target_type,
      `time-series` = hubData::create_timeseries_schema(
        hub_path = hub_path, date_col = date_col, na = na
      ),
      `oracle-output` = hubData::create_oracle_output_schema(
        hub_path = hub_path, na = na
      )
    )
  }
...
guess_target_type_from_path <- function(target_file_path,
