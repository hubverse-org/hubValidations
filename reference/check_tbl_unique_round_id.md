...
object is returned. If no valid `round_id_col` name is provided or can
extracted from config (check through `check_valid_round_id_col`), a
`<message/check_error>` condition class object is returned and the rest
of the check skipped.
