...
* `check_target_tbl_values()` now supports relaxed date validation for time-series target data (#274).
  - When `allow_extra_dates = TRUE` (default), date values are not required to match tasks.json, allowing historical observations while validating other task IDs strictly. This behavior is controlled by the new `allow_extra_dates` parameter.
