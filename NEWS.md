...
- Add argument `compound_taskid_set` to `expand_model_out_grid()` and `submission_tmpl()` that allows users to override the `compound_taskid_set()` when creating sample indices in the `output_type_id` column of samples.
