...
* `check_tbl_values()` now validates derived task ID columns like any other task ID column, instead of ignoring them. A value the config does not list for a derived task ID is reported as an invalid value in that column. A value that only another modeling task lists is reported as an invalid combination, where the previous check accepted any value listed anywhere in the round. The `derived_task_ids` argument of `check_tbl_values()` is deprecated and has no effect. Note that a derived task ID value is still not checked against the task IDs it is derived from, so extra optional or custom checks to validate task ID relationships are still recommended (#378).
