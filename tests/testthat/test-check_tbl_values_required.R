test_that("check_tbl_values_required works with 1 model task & completely opt cols", {
  ...
  expect_equal(
    missing_req_block$missing,
    tbl_hub[1:23, names(tbl_hub) != "value"]
  )
  ...
  expect_equal(
    res_missing_otid$missing,
    tbl_hub[24:26, names(tbl_hub) != "value"]
  )
})
...
test_that("check_tbl_values_required works with 2 separate model tasks & completely missing cols", {
  ...
  expect_equal(
    ...
    tbl_hub[24:25, names(tbl_hub) != "value"]
  )
...
test_that("check_tbl_values_required correctly matches numeric output type IDs when output type ID col is character.", {
...
test_that("check_tbl_values_required works when config contains non required modeling task.", {
...
test_that("Missing required modeling task detected (#203)", {
  ...
  res <- check_tbl_values_required(
    tbl_chr,
    round_id = "2022-11-05",
    file_path = "missing-req-vals-203.parquet",
    hub_path = "test-hub",
    derived_task_ids = "target_end_date"
  )
