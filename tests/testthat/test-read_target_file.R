...
test_that("read_target_file works with single timeseries file.", {
  ...
  expect_equal(names(file), c("date", "target", "location", "observation"))
  expect_equal(dim(file), c(20510L, 4L))
  expect_equal(
  ...
  chr_file <- read_target_file("time-series.csv", hub_path, coerce_types = "chr")
  expect_s3_class(chr_file, "tbl_df")
  expect_equal(dim(chr_file), c(20510L, 4L))
  expect_equal(
    purrr::map_chr(chr_file, ~ class(.x)),
    c(
      date = "character", target = "character", location = "character",
...
test_that("read_target_file works with single oracle-output file.", {
  ...
  expect_equal(dim(file), c(200340L, 6L))
  expect_equal(
    purrr::map_chr(file, ~ class(.x)),
    c(
  ...
  chr_file <- read_target_file("oracle-output.csv", hub_path,
    coerce_types = "chr"
  )
  expect_s3_class(chr_file, "tbl_df")
  expect_equal(dim(chr_file), c(200340L, 6L))
  expect_true(all(is.na(file$output_type_id[file$output_type == "quantile"])))
  expect_equal(
    purrr::map_chr(chr_file, ~ class(.x)),
    c(
      location = "character", target_end_date = "character", target = "character",
      output_type = "character", output_type_id = "character", oracle_value = "character"
    )
  )
})
...
test_that("read_target_file works with split timeseries file.", {
  ...
  fs::dir_create(dir)
  split(dat, dat$target) |> purrr::iwalk(
    ~ {
      target <- gsub(" ", "_", .y)
  ...
  expect_s3_class(file, "tbl_df")
  expect_equal(names(file), c("date", "target", "location", "observation"))
  expect_equal(dim(file), c(10255, 4L))
  expect_equal(
    purrr::map_chr(file, ~ class(.x)),
    c(
      date = "Date", target = "character", location = "character",
  ...
  chr_file <- read_target_file("time-series/target-wk_inc_flu_hosp.csv",
    dir_hub_path,
    coerce_types = "chr"
  )
  expect_s3_class(chr_file, "tbl_df")
  expect_equal(dim(chr_file), c(10255, 4L))
  expect_equal(
    purrr::map_chr(chr_file, ~ class(.x)),
    c(
      date = "character", target = "character", location = "character",
...
test_that("connect_target_timeseries with HIVE-PARTTIONED data works on local hub", {
  ...
  # read in one of the time-series file with hub schema
  file <- read_target_file("time-series/target=wk%20inc%20flu%20hosp/part-0.parquet",
    hub_path = dir_hub_path
  )
  expected_dims <- c(10255, 4L)
  expect_s3_class(file, "tbl_df")
  expect_equal(names(file), c("date", "location", "observation", "target"))
  ...
  file_chr <- read_target_file("time-series/target=wk%20inc%20flu%20hosp/part-0.parquet",
    hub_path = dir_hub_path,
    coerce_types = "chr"
  )
  ...
  # read in one of the time-series file as all character
  file_chr <- read_target_file("time-series/target=wk%20inc%20flu%20hosp/part-0.csv",
    hub_path = dir_hub_path,
    coerce_types = "chr"
  )
  expect_equal(dim(file_chr), expected_dims)
  ...
  # Create CSV a hive partitioned timeseries dataset by date ----
  fs::dir_delete(dir)
  fs::dir_create(dir)
  arrow::write_dataset(dat, dir, partitioning = "date", format = "csv")
  file <- read_target_file("time-series/date=2020-01-11/part-0.csv",
    hub_path = dir_hub_path, date_col = "date"
  )

  expected_dims <- c(16, 4L)
  expect_s3_class(file, "tbl_df")
  expect_equal(dim(file), expected_dims)
  expect_equal(
    purrr::map_chr(file, ~ class(.x)),
    c(
      target = "character", location = "character", observation = "numeric",
      date = "Date"
    )
  )
  expect_equal(
    purrr::map_chr(file, ~ typeof(.x)),
    c(
      target = "character", location = "character", observation = "double",
      date = "double"
    )
  )
  # read in one of the time-series file as all character
  file_chr <- read_target_file("time-series/date=2020-01-11/part-0.csv",
    hub_path = dir_hub_path,
    coerce_types = "chr"
  )
  expect_equal(dim(file_chr), expected_dims)
  expect_equal(
    purrr::map_chr(file_chr, ~ class(.x)),
    c(
      target = "character", location = "character", observation = "character",
      date = "character"
    )
  )
})
